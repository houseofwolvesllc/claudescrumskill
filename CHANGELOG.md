...
### Changed
- **Tiering is now story-aware, with a mandatory floor** (`lib/workflows/_shared/resolve_agent_tier.mjs`, `sprint_pipeline.js`): tiering asked what *kind* of work a stage is but never how *hard* the story is, so a 1-point README edit and a 13-point core-domain story both ran `implement` at the session model. `resolveAgentTier` now takes the story: `implement` runs one tier down at 1–5 points and at the session model for 8–13; `review` runs cheapest at 1–2 and one tier down above; `verify` is always cheapest, since it runs a build/lint/test command and reports a status rather than exercising judgment. The **`implement` floor is mandatory** — never below one tier down at any point value. `points` is an estimate authored before anyone read the code, so a mis-estimated 1-pointer that turns out to be hard degrades gently instead of going to the weakest model; `review` may floor lower because a weak review is caught downstream by `verify` and the tests, whereas a weak implementation *is* the artifact. Two overrides cut against cost for the same reason the retune kept verification guarding destructive operations: `persona: ops` and `priority: P0-critical` never tier down. The story argument is optional — `adversarial_verify` has findings, not stories, and falls back to stage tiering.
