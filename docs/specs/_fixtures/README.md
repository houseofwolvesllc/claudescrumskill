...
> **v2.0.0 note:** Verification cases 1-3 (small_prd, large_prd) now exercise workflow-backed internal execution. Sprint execution goes through `sprint_pipeline.js`, Pass 2 elaboration through `elaborate_epics.js`, and emulation verification through `adversarial_verify.js`. The expected user-visible artifacts are unchanged from v1.8.x; observe the workflow run progress in the Workflow tool UI to confirm the new substrate is in use.
