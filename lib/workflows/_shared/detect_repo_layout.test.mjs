...
// --- an override that costs parallelism announces it -------------------------

import { forcedSerialWarning } from './detect_repo_layout.mjs'

const FORCED_SERIAL_ON_CAPABLE_REPO = {
  requested: 'serial-in-tree',
  detected: 'worktree',
  viableProvisioning: ['clone', 'install'],
}

test('forcing serial where the gate would have chosen worktree warns that stories will not run concurrently', () => {
  const warning = forcedSerialWarning(FORCED_SERIAL_ON_CAPABLE_REPO)

  assert.match(warning, /^WARNING:/)
  assert.match(warning, /ONE AT A TIME/)
  assert.match(warning, /omit isolationStrategy/)
})

test('the warning names the provisioning that was available, so the claim is checkable', () => {
  assert.match(forcedSerialWarning(FORCED_SERIAL_ON_CAPABLE_REPO), /clone, install/)
})

test('a repo whose deps are tracked needs no provisioning, and the warning says so rather than naming none', () => {
  const warning = forcedSerialWarning({ ...FORCED_SERIAL_ON_CAPABLE_REPO, viableProvisioning: [] })

  assert.match(warning, /dependencies already tracked/)
  assert.doesNotMatch(warning, /provisioning: \)/)
})

test('forcing serial where the gate would also have chosen serial costs nothing and says nothing', () => {
  assert.equal(forcedSerialWarning({ requested: 'serial-in-tree', detected: 'serial-in-tree' }), '')
})

test('forcing worktree is not this warning — the other one covers it', () => {
  assert.equal(forcedSerialWarning({ requested: 'worktree', detected: 'worktree' }), '')
  assert.equal(forcedSerialWarning({ requested: 'worktree', detected: 'serial-in-tree' }), '')
})

test('an omitted override is not an override, so it is never warned about', () => {
  assert.equal(forcedSerialWarning({ requested: 'auto', detected: 'worktree' }), '')
  assert.equal(forcedSerialWarning({ requested: undefined, detected: 'worktree' }), '')
})
