...
// claims to have written it. This is the executable specification of the
// Phase 2/3 completion gate in skills/project-orchestrate/SKILL.md: that gate
// runs in the skill markdown because it needs a filesystem the Workflow runtime
// cannot reach (ADR-0006), and this module states the same rule as code so the
// semantics stay unit-tested by artifact_freshness.test.mjs. It is deliberately
// not inlined into any workflow script and not in the inline manifest (see
// EM-201): the comparison it owns has no reachable caller inside the runtime.
//
