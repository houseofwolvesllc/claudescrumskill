...
test('an ops 1-point story implements at the session model despite the difficulty rules', () => {
  const implementTier = story => resolveAgentTier('implement', { sessionModel: SESSION_MODEL, story })
  assert.deepEqual(implementTier({ ...OPS_STORY, points: 1 }), {})
  assert.deepEqual(implementTier({ points: 1 }), { model: 'sonnet' })
})
