...
const REPO_LAYOUT_PROBE_SCHEMA = {
  type: 'object',
  required: ['stdout', 'mainTreePath', 'rootFiles', 'errored'],
  properties: {
    stdout: { type: 'string' },
...
// Probe the three facts the batch's execution model rests on: whether
// node_modules is tracked, where the main working tree is, and which lockfile
// the repository root carries. The runtime has no child_process (ADR-0006), so
// the git runs are delegated to an agent; the pure resolvers (inlined above)
// turn the answers into strategies. A failed probe is treated as a command
// error → the safe default.
async function probeRepoLayout() {
  phase('Detect')
  const probe = await agent(
    `Report three facts about this repository. A fresh \`git worktree add\` materializes tracked files only, so together they decide how the sprint's stories are executed.

Run \`git ls-files node_modules\` and \`git rev-parse --show-toplevel\` at the repository root, and list the entries directly in that root directory. Return:
...
Do NOT create, modify, or delete anything.`,
  ...
  return probe || { stdout: '', mainTreePath: '', rootFiles: [], errored: true }
}
