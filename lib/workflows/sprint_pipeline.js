...
function buildVerifyPrompt(review, story, isolation) {
  const treeLine =
    isolation === 'worktree'
      ...
      : `You are in the SHARED working tree (serial-in-tree; one story at a time). Check out \`story/${story.slug}\` here (\`git checkout story/${story.slug}\`); the pipeline resets the tree before the next story, so build artifacts are cleaned up between stories.`
...
This reverts tracked changes and clears stray build artifacts (dist/, coverage) while PRESERVING every node_modules (root and nested). Do NOT run a bare \`git clean -fdx\` — its -x would delete the untracked node_modules this strategy reuses. Return { done: true } when the tree is clean.`
}
