...
// }
//
// returns: { stories: SprintStoryReturn[], _telemetry: Interval[] }
//   stories — one per story; failed items filtered to null upstream.
//   _telemetry — out-of-band stage timings (SprintPipelineReturnSchema), one
//                interval per agent() call; consumers iterating stories skip it.
  ...
  properties: {
    ...
    findings: {
      ...
      properties: {
        ...
        warning: { type: 'array', items: FINDING_SCHEMA },
        info: { type: 'array', items: FINDING_SCHEMA },
      },
    },
    summary: { type: 'string' },
  },
...
const VERIFY_RETURN_SCHEMA = {
  ...
  properties: {
    ...
    notes: { type: 'string' },
    // What `git rev-parse HEAD` printed in the tree the stage read, on every
    // path. The stage raises treeIdentityFailure only when it noticed the tree
    // was wrong; this carries the evidence back either way, so the mismatch is
    // settled against the assigned commit in data rather than resting on the
    // stage's own conclusion.
    observedHead: { type: 'string' },
    // What HEAD printed when the tree was not the one the stage was placed at.
    // Present only on that failure, and it is what separates a tree the stage
    // could not read from a story whose files are genuinely missing.
    treeIdentityFailure: { type: 'string' },
  },
}
...
function forcedSerialWarning({ requested, detected, viableProvisioning = [] }) {
...
// >>> BEGIN inlined from _shared/prune_story_worktrees.mjs — DRY source of truth; regenerate via inline_sync, do not hand-edit >>>
...
function selectRemovableWorktrees(entries, { storyBranches = [], isMergedIntoRelease }) {
...
function pruneWorktreeCommands(paths) {
...
function selectOrphanedWorktreeBranches(candidates) {
  return (candidates ?? [])
...
function deleteBranchCommands(names) {
  return names.map(name => ['branch', '-D', name])
}
// <<< END inlined from _shared/prune_story_worktrees.mjs <<<

// >>> BEGIN inlined from _shared/stamp_story_facts.mjs — DRY source of truth; regenerate via inline_sync, do not hand-edit >>>
...
function stampStoryFacts(reported, { storySlug, branch }) {
...
function storyFactDisagreements(reported, { storySlug, branch }) {
  return [
...
// <<< END inlined from _shared/stamp_story_facts.mjs <<<

// >>> BEGIN inlined from _shared/retry_placement.mjs — DRY source of truth; regenerate via inline_sync, do not hand-edit >>>
...
const PLACEMENT_ATTEMPT_LIMIT = 2

function placementAttemptsRemain(attempt, limit = PLACEMENT_ATTEMPT_LIMIT) {
...
function placementRetryNotice({ storySlug, attempt, observed, limit = PLACEMENT_ATTEMPT_LIMIT }) {
  return (
...
function exhaustedPlacementDetail({ observed, attempts = PLACEMENT_ATTEMPT_LIMIT }) {
  return `${observed || 'nothing'} (unchanged across ${attempts} attempts)`
}
// <<< END inlined from _shared/retry_placement.mjs <<<

// >>> BEGIN inlined from _shared/describe_finding.mjs — DRY source of truth; regenerate via inline_sync, do not hand-edit >>>
...
// <<< END inlined from _shared/describe_finding.mjs <<<
...
function treeIdentityFailure({ revision, detail = '' }) {
  return {
    status: INFRASTRUCTURE_FAILED,
    reason:
...
// >>> BEGIN inlined from _shared/assert_tree_identity.mjs — DRY source of truth; regenerate via inline_sync, do not hand-edit >>>
...
function treeIdentityAssertion(headCommit) {
...
function mismatchedHead({ assigned, observed }) {
  if (!assigned || !observed) return NO_MISMATCH
  return sameCommit(assigned, observed) ? NO_MISMATCH : observed
}
// <<< END inlined from _shared/assert_tree_identity.mjs <<<

// >>> BEGIN inlined from _shared/reconcile_story_results.mjs — DRY source of truth; regenerate via inline_sync, do not hand-edit >>>
...
// not.
function reconcileStoryResults({ requested = [], returned = [] }) {
...
// <<< END inlined from _shared/reconcile_story_results.mjs <<<

// >>> BEGIN inlined from _shared/stage_timing.mjs — DRY source of truth; regenerate via inline_sync, do not hand-edit >>>
...
function createStageTimer(agentFn) {
  const timings = []
...
// <<< END inlined from _shared/stage_timing.mjs <<<
...
// Wrap the runtime agent once so every stage call it makes leaves behind one
// timed interval; `timings` is the array the workflow returns under _telemetry.
const { timedAgent, timings } = createStageTimer(agent)

if (!stories || stories.length === 0) {
  log('No stories in sprint — exiting.')
  return { stories: [], _telemetry: timings }
}
...
const WORKTREE_PROBE_SCHEMA = {
  type: 'object',
  required: ['porcelain', 'mergedShas'],
  properties: {
    ...
    worktreeBranches: {
      ...
      items: {
        type: 'object',
        required: ['name', 'containingRefs'],
        properties: {
          name: { type: 'string' },
          containingRefs: { type: 'array', items: { type: 'string' } },
        },
      },
    },
  },
...
function resolveIsolationStrategy(probe, viableProvisioning) {
  ...
  if (isolationStrategy === WORKTREE || isolationStrategy === SERIAL_IN_TREE) {
    ...
    // An override suppresses the gate. When that costs the run its parallelism,
    // the run says so — an instruction that is obeyed silently is one nobody
    // knows was given.
    const surrendered = forcedSerialWarning({
      requested: isolationStrategy,
      detected,
      viableProvisioning,
    })
    if (surrendered) log(surrendered)
...
function buildReviewPrompt(impl, story) {
  const branch = storyBranch(story)
  return `You are reviewing the implementation of story ${story.slug} on branch ${branch}.

Read the diff between ${releaseBranch} and ${branch} with \`git diff ${releaseBranch}...${branch}\` — do NOT check out either branch (review compares refs only; checking out would disturb the working tree the next stage merges on).
...
// The last commit implement reported, which is the branch head because implement
// is asked for its commits oldest-first. Empty when it reported none.
function implementHeadCommit(impl) {
  return impl.commits?.at(-1) || ''
}
...
function verifyRevision(impl, branch) {
  return implementHeadCommit(impl) || branch
}

function buildVerifyPrompt(impl, story, isolation) {
  const branch = storyBranch(story)
  const revision = verifyRevision(impl, branch)
  const treeIdentityLine = treeIdentityAssertion(implementHeadCommit(impl))
  const dependencyInstruction = storyProvisioning.get(story.slug).instruction
  const treeLine =
    isolation === 'worktree'
...
// The PR stage returns the WHOLE story record, so a story's slug and branch came
// back as whatever that agent chose — though the pipeline assigned both before
// it ran, and a stage that merges naturally reports the branch it merged INTO.
// Take back the two facts the pipeline owns. Status, commits and blockers stay
// the agent's: it is the only one that watched the merge.
//
// The disagreement is reported, not silently overwritten — a silent correction
// hides the drift that produced it.
function ownStoryFacts(story, reported) {
  if (!reported) return reported
  const assigned = { storySlug: story.slug, branch: storyBranch(story) }
  for (const drift of storyFactDisagreements(reported, assigned)) {
    log(`Story ${story.slug}: record corrected — ${drift}.`)
  }
  return stampStoryFacts(reported, assigned)
}
...
function reviewBlocked(story, impl, review) {
  return makeSprintStoryReturn({
    ...
    status: 'blocked',
    branch: storyBranch(story),
    commits: impl.commits,
    blockers: findingLabels(review.findings),
...
// The verify stage read a tree that is not this story's, so it has discovered
// where the harness put it, not whether the code works. That is the same
// infrastructure failure a worktree without dependencies reports, and it is
// reported here rather than as a blocked story so nobody reads 'the directory is
// empty' as a finding about files this story wrote.
function treeIdentityFailed(story, impl, observedHead) {
  return makeSprintStoryReturn({
    storySlug: story.slug,
    branch: storyBranch(story),
    commits: impl.commits,
    ...treeIdentityFailure({
      revision: verifyRevision(impl, storyBranch(story)),
      detail: observedHead,
    }),
  })
}

// What HEAD printed when the tree the verify stage read was not the story's, and
// empty when it was. The SHA it carried back is compared against the commit it
// was placed at first: a stage that describes the wrong tree is one that did not
// notice it was in the wrong tree, so its own report is the second source here,
// not the only one.
function wrongTreeHead(impl, verify) {
  return (
    mismatchedHead({ assigned: implementHeadCommit(impl), observed: verify.observedHead }) ||
    verify.treeIdentityFailure ||
    ''
  )
}
...
async function runStoryChain(story, isolation) {
  const agentIsolation = isolationOption(isolation)

  const impl = await timedAgent(buildImplementPrompt(story, isolation), {
  ...
  const review = await timedAgent(buildReviewPrompt(impl, story), {
  ...
  const { verify, misplacement } = await verifyStory(story, impl, isolation)
  if (misplacement) {
    return treeIdentityFailed(story, impl, exhaustedPlacementDetail({ observed: misplacement }))
  }
  if (!verify) return null
  if (verify.verifyStatus === 'fail') return verifyBlocked(story, impl, verify)

  const finalize = () =>
    timedAgent(buildOpenPRPrompt(verify, story, impl), {
      label: `pr:${story.slug}`,
      phase: 'Open PR',
      schema: SPRINT_STORY_RETURN_SCHEMA,
      ...resolveAgentTier('pr', { sessionModel }),
    }).then(reported => ownStoryFacts(story, reported))
...
function isolationOption(isolation) {
  return isolation === 'worktree' ? { isolation: 'worktree' } : {}
}

// Verification is given a second tree before its misplacement is called a
// failure. The harness parks every new worktree at the repository's default
// branch and the stage repositions itself; one that failed to reposition says
// nothing about whether the next will, and in worktree mode each attempt gets a
// FRESH tree, so the retry is a different placement rather than a repeat.
//
// Safe here because verification only READS. Implement commits, so a dependency
// setup failure is deliberately NOT retried — a second attempt there could leave
// a story with two sets of commits.
//
// Returns the verification record, or the misplacement that outlived the
// attempts. Never both.
async function verifyStory(story, impl, isolation) {
  let misplacement = ''
  for (let attempt = 1; attempt <= PLACEMENT_ATTEMPT_LIMIT; attempt += 1) {
    const verify = await timedAgent(buildVerifyPrompt(impl, story, isolation), {
    ...
    if (!verify) return { verify: null, misplacement: '' }

    misplacement = wrongTreeHead(impl, verify)
    if (!misplacement) return { verify, misplacement: '' }
    if (placementAttemptsRemain(attempt)) {
      log(placementRetryNotice({ storySlug: story.slug, attempt, observed: misplacement }))
    }
  }
  return { verify: null, misplacement }
}
...
// Every result the batch has, reconciled against the stories it was asked to run
// before it goes back. A stage that returned nothing leaves a shorter array and
// nothing else — the same shape a completed batch returns — so the two sets are
// compared here, on the one path both execution models return through, and the
// batch says which of the two it was. The story results ride under `stories`;
// the run's stage timings ride alongside them under the out-of-band `_telemetry`
// field, which consumers iterating story results skip.
function reconciledResults(results) {
  const returned = results.filter(Boolean)
  log(reconcileStoryResults({ requested: stories, returned }))
  return { stories: returned, _telemetry: timings }
}
...
if (resolvedIsolation === 'serial-in-tree') {
  ...
  const results = await runSequential(ordered, {
    ...
    resetBetween: (prevStory, nextStory) =>
      timedAgent(buildResetPrompt(prevStory, nextStory), {
  ...
  return reconciledResults(results)
}
...
// Worktree mode leaves two worktrees per story — one holding the story branch,
// one detached at the commit verify checked out — and until now nothing
// reclaimed them. The Workflow tool releases a worktree only when it is
// unchanged, and a story's worktree always carries commits, so sprints
// accumulated worktrees and the disk they held until someone noticed.
//
// Selection runs HERE, over the listing git actually reports, rather than being
// described to an agent and taken on trust (ADR-0009). The probe agent reports
// facts, selectRemovableWorktrees decides, and the removal agent is handed exact
// paths and no discretion. Teardown runs only after every merge has landed, so
// the ancestry test the rule leans on is meaningful.
async function reclaimStoryWorktrees(results) {
  const probe = await timedAgent(buildWorktreeProbePrompt(), {
    label: 'teardown:scan',
    phase: 'Teardown',
    schema: WORKTREE_PROBE_SCHEMA,
    ...resolveAgentTier('teardown', { sessionModel }),
  })
  if (!probe) {
    log('Teardown: worktree listing unavailable; worktrees left in place.')
    return
  }

  const removable = removableWorktreePaths(probe, results)
  const orphaned = selectOrphanedWorktreeBranches(probe.worktreeBranches)
  if (removable.length === 0 && orphaned.length === 0) {
    log('Teardown: no reclaimable worktree or orphaned harness ref from this sprint.')
    return
  }

  await timedAgent(buildTeardownPrompt(removable, orphaned), {
    label: 'teardown:remove',
    phase: 'Teardown',
    schema: TEARDOWN_RETURN_SCHEMA,
    ...resolveAgentTier('teardown', { sessionModel }),
  })
  log(
    `Teardown: reclaimed ${removable.length} worktree(s) and ${orphaned.length} orphaned harness ref(s) ` +
      `from this sprint — ${[...removable, ...orphaned].join(', ')}.`,
  )
}

function removableWorktreePaths(probe, results) {
  const merged = new Set(probe.mergedShas ?? [])
  return selectRemovableWorktrees(parseWorktreeList(probe.porcelain), {
    storyBranches: landedStoryBranches(results),
    isMergedIntoRelease: sha => merged.has(sha),
  })
}

// Only a story that landed gives up its worktree; a story that failed keeps one
// so the failure stays inspectable.
function landedStoryBranches(results) {
  const landed = new Set(results.filter(result => result?.status === 'done').map(result => result.storySlug))
  return stories.filter(story => landed.has(story.slug)).map(storyBranch)
}

function buildWorktreeProbePrompt() {
  return `Report the repository's worktrees. Run exactly these, at the repository root, and report what they print — do not remove, prune, or modify anything.

  git worktree list --porcelain

Then, for EVERY worktree that block reports as \`detached\`, test whether its HEAD commit already sits on the release branch:

  git merge-base --is-ancestor <that worktree's HEAD sha> ${releaseBranch}

Exit status 0 means yes, non-zero means no.

Then list the harness's own branch refs and, for each, every ref that also contains its commit:

  git branch --list 'worktree-*' --format='%(refname:short)'
  git branch -a --contains <each name from that list> --format='%(refname:short)'

Return { porcelain, mergedShas, worktreeBranches }. porcelain is the VERBATIM stdout of the first command. mergedShas lists every detached HEAD sha that answered yes. worktreeBranches has one entry per name the branch listing printed, each { name, containingRefs } where containingRefs is what the second command printed for that name, with the name itself included as git prints it. Report only what the commands printed; do not infer a sha or a ref you did not see.`
}

function buildTeardownPrompt(paths, branches) {
  const commands = [
    ...pruneWorktreeCommands(paths).map(renderGitCommand),
    ...deleteBranchCommands(branches).map(renderGitCommand),
  ]
  return `Sprint teardown: reclaim this sprint's worktrees and the harness refs they left behind. Run these commands at the repository root, in EXACTLY this order — the worktrees go first, because a branch still checked out in a worktree cannot be deleted:
${commands.map(command => `  ${command}`).join('\n')}

These paths were selected by the pipeline from git's own worktree listing. Run the list as given: do not add paths, do not remove a worktree that is not listed, and never run a bare \`git worktree prune\` against anything else — other sprints and other agents hold worktrees in this same repository. Removing a worktree deletes its checkout only; the commits stay reachable from the story branch. Each branch deletion was selected because another ref already contains that commit, so the commit survives the ref. Return { removed, deletedBranches }.`
}

const results = await Promise.all(stories.map(story => terminal.get(story.slug)))
await reclaimStoryWorktrees(results)
return reconciledResults(results)
