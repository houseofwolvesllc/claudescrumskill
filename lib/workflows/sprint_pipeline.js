...
async function probeRepoLayout() {
  ...
  const probe = await agent(
    `Report seven facts about this repository and the machine it sits on. A fresh \`git worktree add\` materializes tracked files only, so the first four decide how the sprint's stories are executed; the last three decide how many of them run at once.
...
Report 0 for any of the last three you could not measure — do not estimate one.
...
function buildImplementPrompt(story, isolation) {
  ...
  const returnLine = `Implement. Commit with a clear message. Return the branch name, the commit SHAs, the files the story changed (\`git diff --name-only ${releaseBranch}...${branch}\`), and any notes.`
...
function buildVerifyPrompt(review, story, isolation) {
  ...
  const treeLine =
    isolation === 'worktree'
      ? `You are in an isolated git worktree — check out \`${branch}\` here (\`git checkout ${branch}\`) so any build artifacts stay off the shared working tree.`
      : `You are in the SHARED working tree (serial-in-tree; one story at a time). Check out \`${branch}\` here (\`git checkout ${branch}\`); the pipeline resets the tree (tracked changes reverted, untracked non-ignored cruft cleared) before the next story.`
