...
async function reclaimStoryWorktrees(results) {
  ...
  if (removable.length === 0) {
    log('Teardown: no reclaimable worktree from this sprint.')
    return
  }

  await agent(buildTeardownPrompt(removable), {
    label: 'teardown:remove',
    phase: 'Teardown',
    schema: TEARDOWN_RETURN_SCHEMA,
    ...resolveAgentTier('teardown', { sessionModel }),
  })
  log(`Teardown: reclaimed ${removable.length} worktree(s) from this sprint — ${removable.join(', ')}.`)
}
...
Exit status 0 means yes, non-zero means no. Return { porcelain, mergedShas } where porcelain is the VERBATIM stdout of the first command and mergedShas lists every detached HEAD sha that answered yes. Report only what the commands printed; do not infer a sha you did not test.`
}

function buildTeardownPrompt(paths) {
  const commands = pruneWorktreeCommands(paths).map(renderGitCommand)
  return `Sprint teardown: reclaim this sprint's worktrees. Run these commands at the repository root, in EXACTLY this order:
${commands.map(command => `  ${command}`).join('\n')}

These paths were selected by the pipeline from git's own worktree listing. Run the list as given: do not add paths, do not remove a worktree that is not listed, and never run a bare \`git worktree prune\` against anything else — other sprints and other agents hold worktrees in this same repository. Removing a worktree deletes its checkout only; the commits stay reachable from the story branch. Return { removed } listing the paths that are gone.`
}
